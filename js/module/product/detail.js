...
$('.detailTab .detailNavi a').click(function(e){
    e.preventDefault();
    $('.detailContent > div').hide().filter(this.hash).show();
    $('.detailTab .detailNavi a').removeClass('active');
    $(this).addClass('active');
}).filter(':eq(0)').click();

$(window).scroll(function () {
    const $tabProduct = $('.detailNavi');
    const $removeEvent = $(".detailNaviBox").offset().top;
    const $scrollTop = $(window).scrollTop();
    const $headerHeight = $("header").height();
    const $tabProductHeight = $tabProduct.offset().top - $headerHeight;
    const fixedEvent = "fixed";
    
    if (!$tabProduct.hasClass(fixedEvent) && $scrollTop >= $tabProductHeight) {
...
//     if(wheel > 0){
//         $(".xans-product-additional .detailNavi").addClass('active'); /* 마우스 올릴때 */
//     } else {
//         $(".xans-product-additional .detailNavi").removeClass('active'); /* 마우스 내릴때 */
//     }
// });